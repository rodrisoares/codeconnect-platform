...
export class CommentsController {
  ...
  @Get('post/:postId')
  @ApiOperation({ summary: 'Listar comentários de um post (paginado)' })
  @ApiParam({ name: 'postId', description: 'ID do post', type: 'number' })
  ...
  findAllByPost(
    ...
    const limitNum = limit ? Math.min(50, parseInt(limit, 10) || 10) : 10;
  ...
  @Post(':id/like')
  @ApiOperation({ summary: 'Curtir/descurtir um comentário (toggle)' })
  @ApiParam({ name: 'id', description: 'ID do comentário', type: 'number' })
  @ApiResponse({
    status: 201,
    description: 'Estado da curtida alternado (retorna likes e likedByMe)',
  })
  ...
  toggleLike(
    ...
    return this.commentsService.toggleLike(id, req.user.sub);
  }
  ...
  @ApiResponse({ status: 404, description: 'Comentário não encontrado' })
  @ApiResponse({ status: 403, description: 'Você não é o autor do comentário' })
  @ApiResponse({ status: 401, description: 'Token inválido ou não fornecido' })
  update(
    @Param('id', ParseIntPipe) id: number,
